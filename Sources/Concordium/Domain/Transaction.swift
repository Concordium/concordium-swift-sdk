...
extension TransactionType: FromGRPC {
    ...
    static func fromGRPC(_ g: GRPC) throws -> TransactionType {
        ...
        case .pltTokenUpdate: return .transferWithMemo
        }
    }
}
...
//    case transferPLT(ticker: String, amount: Amount, receiver: AccountAddress, memo: Memo? = nil)
    case tokenUpdate(tokenId: String, operation: TokenUpdateOperation)
}
...
extension AccountTransactionPayload: Serialize, Deserialize {
    @discardableResult public func serialize(into buffer: inout ByteBuffer) -> Int {
        ...
        case let .configureDelegation(data):
            ...
            res += buffer.writeSerializable(data)
        case let .tokenUpdate(tokenId, operation):
            res += buffer.writeSerializable(TransactionType.update)
...
extension CBOR {
    /// Creates an ordered CBOR map using indefinite-length encoding (starts with 0xBF ... ends with 0xFF)
    static func orderedMap(_ pairs: [(CBOR, CBOR)]) -> CBOR {
        var encoded: [UInt8] = [0xBF] // map(*)
        for (key, value) in pairs {
            encoded.append(contentsOf: key.encode())
            encoded.append(contentsOf: value.encode())
        }
        encoded.append(0xFF) // break
        return try! CBOR.decode(encoded) ?? .undefined
    }
}
